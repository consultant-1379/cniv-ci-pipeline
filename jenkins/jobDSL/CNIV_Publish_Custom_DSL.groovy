...
def createPipelineJob(env_map) {
    ...
    pipelineJob(pipeline_name) {
        ...
        parameters {
            ...
            stringParam('INT_CHART_NAME', '', 'Dependency chart name. Used for Spinnaker pipeline only (not required for standard build of the job)')
            stringParam('INT_CHART_VERSION', '', 'Dependency chart version. Used for Spinnaker pipeline only (not required for standard build of the job)')
        }
        ...
        definition {
            cps {
                script(readFileFromWorkspace('jenkins/jenkinsFiles/jenkinsfile-cniv-integration-publish-custom.groovy'))
                sandbox(true)
            }
        }
    }
