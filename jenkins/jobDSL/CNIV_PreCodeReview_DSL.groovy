...
def createPipelineJob(env_map) {
    ...
    pipeline_name = git_repo_path.split('/').last() + '_PreCodeReview'
    ...
    pipelineJob(pipeline_name) {
        ...
        definition {
            cps {
                script(readFileFromWorkspace('jenkins/jenkinsFiles/jenkinsfile-cniv-precodereview.groovy'))
                sandbox(true)
            }
        }
    }
